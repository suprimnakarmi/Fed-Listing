...
class AuxClient(NumPyClient):
    def __init__(self, cid: int, data: Data, train_idx: torch.Tensor):
        ...
        print("NUM of classes: ", int(data.y.max().item()) + 1)
...
def build_client_dataset(cid: int):
    ...
    gperm = torch.load("aux_permutation.pt") if os.path.exists("aux_permutation.pt") else None
    if gperm is None:
...
def build_client_dataset_equal(cid: int):
    aux = torch.load(AUX_PATH, weights_only=False)
    y = aux.y
    num_classes = int(y.max().item()) + 1
    class_indices = [torch.where(y == i)[0] for i in range(num_classes)]

    # Number of clients and class samples per client
    min_class_size = min([len(idx) for idx in class_indices])
    samples_per_class = min_class_size // NUM_CLIENTS

    selected_indices = []

    for class_idx in class_indices:
        class_idx = class_idx[torch.randperm(len(class_idx))]  # shuffle
        start = cid * samples_per_class
        end = start + samples_per_class
        selected_indices.append(class_idx[start:end])

    client_node_ids = torch.cat(selected_indices, dim=0)

    # Build induced subgraph for client
    eidx, _ = subgraph(client_node_ids, aux.edge_index, relabel_nodes=True, num_nodes=aux.num_nodes)
    d = Data(x=aux.x[client_node_ids],
             y=aux.y[client_node_ids],
             edge_index=eidx)
    ...
    print(f"[Client {cid}] saved equal label distribution {out_path}") 
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    # print("ARGS", args.cid)
    if args.cid == 0 or args.cid == 6 or args.cid == 9:
        # print("Here")
        data, idx = build_client_dataset_equal(args.cid)
    else:
